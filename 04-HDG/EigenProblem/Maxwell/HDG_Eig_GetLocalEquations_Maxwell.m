function [List_LocSol, List_LocSol_f, List_Ns]...
    ...
    %
    %          wh                    uhat_t_Fi       0
    %  M_loc* (uh  ) = Sum: N_i * ?          )   +( Id )* lamda* uh
    %          ph                    phat_Fi         0
    % >>
    %
    %  wh                  uhat_t_Fi
    %  uh   = Sum: Loc_i *                 + Loc_f * lamda* uh
    %  ph                   phat_Fi
    %
    %%%%
    
    List_LocSol_f = zeros(N_local,Nu,num_elements,numeric_t); % Local solver for source_j_1 and source_j_2
    ...
    temp_mat1 = zeros(Nw,Nu,numeric_t);
    temp_mat2 = eye(Nu,Nu,numeric_t);
    temp_mat = [temp_mat1;temp_mat2;temp_mat1];
