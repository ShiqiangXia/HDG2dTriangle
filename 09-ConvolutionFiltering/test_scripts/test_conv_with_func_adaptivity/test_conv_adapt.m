function test_conv_adapt(para,Ncoarse, N_outer_adap_steps )
    ...
    if flag_conv == 1
        ...
        % minimal number of elements from the boundary 
        if poly_k ==1
    ...
    if strcmp(pb_type(1),'1')
        ...
        for jj = 1: Ncoarse
            ...
            % Solve the primal and dual problem by HDG method on Triangluar
            % mesh with Niter adaptive stpes. We choose Niter=1, namely no
            % adaptive refine here
            for ii = 1:Niter
                HDG_start_time = cputime;
                cprintf('blue','Mesh %d ... \n',ii)
                ...
                % DOF of the initial triangular mesh
                ...
                if strcmp(pb_type(2),'0') % source problem
                    ...
                    if err_cal_flag==1
                        ...
                        err_cal_time = cputime - err_cal_start_time;
                    end
...
%                 end
                ...
                HDG_total_time = (cputime - HDG_start_time) - err_cal_time;
        ...
        %----------------------------------------------------------------------
            ...
            % ------------------------------------------------------------
            if para.post_process_flag == 1
                % project HDG local processed solution to square mesh
...
%                 
            ...
            if flag_conv == 1
                ...
                if para.post_process_flag == 1
                    % convolution filter local porcessed solution
...
%                     GQ1DRef_pts,GQ1DRef_wts, hx, hy);
                ...
                [Jh_star_inner, ACh_star_inner, Jh_inner_elewise, ACh_inner_elewise] = ...
                    LinearFunctional_inner_gradient(pb_type(4),pb_type(3),...
